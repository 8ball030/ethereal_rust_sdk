...
async fn test_simple_order() {
    let client = common::create_test_client().await.expect("Failed to create client");
...
async fn test_cancel_all_orders() {
    ...
    let orders = client.order().list_by_subaccount_id(params).await.expect("Failed to list orders");
    if orders.data.is_empty() {
        return;
    }
    let order_ids = orders.data.iter().map(|order| order.id.clone().to_string()).collect();
